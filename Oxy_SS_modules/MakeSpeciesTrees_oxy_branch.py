...
# last edit abigailc@Leviathan july5 2017
...
#always run in /Users/abigailc/Documents/MakeSpeciesTrees on abigailc@Actaeon
    ...
    def gen_species_lists(self):
        ...
        for item in self.ids:
            ...
            if "|" in taxon:
                ...
                taxon = tlist[-1]
                #-1 is default.
                #replaced with: whatever precedes the |gi| since most of my formats have that.
...
###############
#taxonomy parsing stuff


#new function
#to deal with the final pass-through

def Species_Tree_For_The_Subsampled_Fasta(ss_subtree_object, projectname, cat_file, individual_party = False):
    ...
    #use bac maybe?? unclear efficiency.
    Fix_Your_Mistakes([ss_subtree_object])
    
    #currently a wrapper for all the running things functions.
    print("MakeSpeciesTree: Checking if files already exist")
    to_add_to_rax_lists, to_add_to_cc_files, clade_to_align, existing_rax_clades = Check_For_Existance([ss_subtree_object], projectname)

    #potential error if species trees completed but gene trees didnt!!!
    print("MakeSpeciesTree: Beginning tree-making")

    #change this to not do any taxonomy / rooting / loss addition BS
    cc_files, rax_out, results_list = Run_Everything_On_SS_Tree(clade_to_align, to_add_to_cc_files, to_add_to_rax_lists, projectname)
    
    print("MakeSpeciesTree: Matching trees to corresponding clades")
...
#wtf this is called from somewhere else??
 #all species trees are complete!

 # File "Overall_DTL_Detector.py", line 608, in <module>
 # File "Overall_DTL_Detector.py", line 230, in Run_Detection
  
def Overall_Species_Trees(list_of_big_clades, projectname, Euk_Gene_List, Bac_Gene_List, Arc_Gene_List, All_Gene_List, individual_party=False):
    
    Set_Fasta_Species_List_And_Category(list_of_big_clades, Euk_Gene_List, Bac_Gene_List, Arc_Gene_List, All_Gene_List)

    Fix_Your_Mistakes(list_of_big_clades)
    
    #currently a wrapper for all the running things functions.
    print("Checking if files already exist")
    to_add_to_rax_lists, to_add_to_cc_files, clade_to_align, existing_rax_clades = Check_For_Existance(list_of_big_clades, projectname)

    #potential error if species trees completed but gene trees didnt!!!
    print("Beginning tree-making")
    cc_files, rax_out, results_list = Run_Everything(clade_to_align, to_add_to_cc_files, to_add_to_rax_lists, projectname)
    print("Matching trees to corresponding clades")
    incomplete = Correlate_Tree_Subtree(rax_out, list_of_big_clades, cc_files)
    for item in clade_to_align:
        if item in incomplete:
            clade_to_align.remove(item)
    if individual_party is False:
        boots_list = Get_Attach_Boots(list_of_big_clades, clade_to_align, projectname)
    else:
        boots_list = "invalid"
    print("Done with everything")
    return rax_out, cc_files, incomplete, boots_list
...
def Get_Attach_Boots(list_of_big_clades, clade_to_align, projectname):
    ...
    for item in list_of_big_clades:
        if item in clade_to_align:
            ...
            os.system("scp "+clus_head+"Species_Trees/"+projectname+"/"+boot_name+" "+projectname+"/Species_Trees/boots/"+boot_name)
            #scp the boot home
...
def Set_Fasta_Species_List_And_Category(list_of_big_clades, Euk_Gene_List, Bac_Gene_List, Arc_Gene_List, All_Gene_List):
    ...
    #done
        ...
        #list_of_inputs = [species_list, cat_file, subtree_ob.ret_string(), subtree_ob.ret_prefix()]
        #sp_tree_gen_list.append(list_of_inputs)
    #as far as I can tell, sp_tree_gen_list is just a less-efficient way of passing ALL the information stored in subtree_obs
    #can we just pass the list of subtree_obs?
    #I think so.
    #set category file to a class variable
        

    #run species tree generator
    #print("Beginning to make species trees... ")
...
def Run_Everything(clade_to_align, to_add_to_cc_files, to_add_to_rax_lists, projectname):
    #currently a wrapper for all the running things functions.
    ...
    if clade_to_align == []:
        ...
        print("Retreiving acceptable outgroup-rooting and loss-candidate species names... may take some time")
        #add outgroup and loss-candidate information for all subtree files. associate roots with subtrees.
        results_list = Get_OG_LOSS_DATA(clade_to_align, projectname)
        #adds loss_species_list, root_species, and species_list_plus_og_loss values to subtree objects.
        
        #cc_files = get_multiple_concat_alignments(sp_tree_gen_list, True, projectname)
        #rax_sp_one  = raxml_run_on_cluster(cc_files, projectname, list_of_roots)
        print("Initializing fastas containing given genes from relevant species")
        cc_files = Get_Sequences(clade_to_align, projectname)
    
        list_of_roots = []
        for item in clade_to_align:
            #because some items might have been removed in the Get_Sequences call if not enough sequences were found...?
            if item.cc_file in cc_files:
                if item.cc_file == "NA":
                    print(item.prefix+" was returned as NA. not running.")
                    continue
                
            else:
                print(item.prefix+" not found in cc_files list. not running.")
                continue
            rs = item.root_species
            
            #this clade will be removed later, in raxml_run_on_cluster.
            if item.cc_file == "NA":
                pass
            elif rs in item.cc_file_obj.ids:
                list_of_roots.append(rs)
            else:
                print("root species is not found in CC file for clade: "+item.prefix)
                print("if removed it will cause off-byone rooting errors. if kept, will error in tree gen.")
                print("exiting")
                raise SystemExit                #
...
def Run_Everything_On_SS_Tree(clade_to_align, to_add_to_cc_files, to_add_to_rax_lists, projectname):
...
def Check_Blasts_Exist(clade_align):
    ...
    for gene_sequences_file in list_of_genelists:
        ...
        #this is a list containing the raw_blast names. not fasta objects, just the NAMES of the files.
...
#Convert_New_Blast_To_Full_Blast
...
def Get_Sequences(clade_list, projectname):
    cc_files_out = []
    for clade in clade_list:
        ...
        for QUERY in gene_name_list:
            ...
            for ENTREZ in clade.species_list_plus_og_loss:
                in_overall = "no"
                no_under = re.sub(" ", "_", ENTREZ.strip("\""))
                #see if its already listed in current_overall_fasta
                for item in current_overall_fasta.species_names:
                    #print(item)
                    if no_under == item:
                        #this is leading to IMPRECISE MATCHING OMG WHY WOULD I DO THIS.
                        #thing not IN ITEM
                        #thing must BE THE ITEM
                        #AHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHH
                        #print(item)
                        in_overall = "yes"
                        index = current_overall_fasta.species_names.index(item)
                ...
                if in_overall == "yes":
                    ...
                    for spec in current_raw_fasta.species_names:
                        if no_under == spec:
                            ...
                            match = True
                            index = current_raw_fasta.species_names.index(spec)
...
def move_to_cluster(list_of_files, clus_path):
    ...
    for item in list_of_files:
        os.system("scp "+item+" "+clus_head[:-1]+clus_path)
...
#to make a species tree on an arbitrary file....
#you need to have a fasta file
#you need to have a project name
#you need to have the blasts done and in correctly named places
#you need to have your fasta in the right place

#could move your shit to place, move results back. ?
def set_up_an_independent_run(fasta, projectname, directory_to_run_in):
    os.chdir(directory_to_run_in)

    from Classes_DTL_Detector.py import *

    rax_out, cc_files, incomplete, boots_species  = Species_Tree_For_The_Subsampled_Fasta(ss_subtree_object, projectname, cat_file)
   
    
    Subtree = Subtree(projectname)
    Subtree.fasta = fasta
    Subtree.fasta_object = Fasta(fasta)
    Subtree.fasta_object.gen_original_lists(fasta)

    Subtree.fasta_object.prefix = projectname
    Subtree.prefix = projectname
    Subtree.projectname = projectname
    ...
    Subtree.cat_file = cat_file

    rax_out, cc_files, incomplete, boots_species  = Species_Tree_For_The_Subsampled_Fasta(Subtree, projectname, cat_file, True)
...
if __name__ == "__main__":
    ...
    if args.projectname is False:
        print("...specificity?")
        raise SystemExit
    set_up_an_independent_run(args.fasta, args.projectname, args.directory)
